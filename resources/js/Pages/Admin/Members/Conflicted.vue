...
const statCards = computed(() => [
    {
        label: 'Total Conflicted Members',
...
const temporaryFilter = ref(props.filters?.temporary || 'all')
...
const updateFilters = () => {
    router.get(route('admin.members.index'), {
        ...
        temporary: temporaryFilter.value,
...
watchDebounced(
    [search, statusFilter, temporaryFilter, passwordFilter, loanFilter, dateFilter, fromDate, toDate, perPage],
    () => {
        updateFilters()
    },
...
const submitImport = () => {
    importForm.post(route('admin.members.import.process'), {
        onSuccess: () => {
...
const reassignSearch = reactive({})
const assigneeSuggestions = reactive({})
const assigneeLoading = reactive({})
...
const fetchAssigneeSuggestions = async (memberId) => {
    const query = normalize(reassignSearch[memberId] || '')
    assigneeLoading[memberId] = true

    try {
        const response = await window.axios.get(route('admin.members.assignees-search', memberId), {
            params: { q: query },
        })
        assigneeSuggestions[memberId] = response.data
    } catch (error) {
        assigneeSuggestions[memberId] = []
    } finally {
        assigneeLoading[memberId] = false
    }
}

const debounceFetchAssigneeSuggestions = (memberId) => {
    if (suggestionTimers[memberId]) {
        clearTimeout(suggestionTimers[memberId])
    }

    suggestionTimers[memberId] = setTimeout(() => {
        fetchAssigneeSuggestions(memberId)
    }, 250)
}

const ensureAssigneeSuggestions = (memberId) => {
    if (!Array.isArray(assigneeSuggestions[memberId]) || assigneeSuggestions[memberId].length === 0) {
        fetchAssigneeSuggestions(memberId)
    }
}

const currentAssigneeOptions = (member) => {
    return assigneeSuggestions[member.id] || []
}

const reassignMember = (memberId, targetId) => {
    if (!confirm('Reassign details from this temporary member to the selected member?')) return
    router.patch(route('admin.members.reassign', memberId), { target_member_id: targetId }, {
        onSuccess: () => {
            router.reload()
        }
    })
    ...
    <AppLayout>
        <div class="space-y-6">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h2 class="text-2xl font-bold text-foreground">Conflicted Members</h2>
                    <p class="text-sm text-muted-foreground mt-1">
                        Manage all conflicted cooperative members
                    </p>
                </div>
                    ...
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div class="flex-1 max-w-md">
                            <div class="relative">
                                ...
                                <Input v-model="search" placeholder="Search members..."
                        ...
                        <div class="flex flex-wrap items-center gap-3">
                            ...
                            <!-- Temporary Filter -->
                            <Select v-model="temporaryFilter">
                                <SelectTrigger class="w-[130px] h-10 border-none shadow-sm rounded-xl">
                            ...
                            <table class="w-full text-sm">
                                ...
                                <tbody class="divide-y divide-border/50">
                                    <tr v-for="member in members.data" :key="member.id"
                                        ...
                                        <td class="py-4 px-6 font-medium text-foreground space-y-1">
                                            <div class="flex items-center gap-2">
                                                ...
                                                <Tooltip v-if="member.is_temporary" :delay-duration="0">
                                                    ...
                                                    <TooltipContent>
                                                        This is a temporary member. Use "Reasign" to move their details
                                                        to a real
                                                        member.
                                                    </TooltipContent>
                                                </Tooltip>
                                        ...
                                        <td class="py-4 px-6">
                                            <Badge :variant="loanStatusVariant(member.loan_status)"
                                                class="rounded-lg px-2 py-0.5 text-[10px] uppercase tracking-wider font-bold">
                                                {{ member.loan_status }}
                                            </Badge>
                                        </td>
                                        ...
                                        <td class="py-4 px-6 text-right">
                                            <Button variant="ghost" size="sm" class="h-8 rounded-lg hover:bg-muted"
                                                as-child>
                                                ...
                                                <DropdownMenu>
                                                    <DropdownMenuTrigger as-child>
                                                        <Button size="sm" variant="outline"
                                                            @click="ensureAssigneeSuggestions(member.id)">Reasign</Button>
                                                    </DropdownMenuTrigger>
                                                    <DropdownMenuContent side="bottom" align="end" class="w-80">
                                                        <DropdownMenuLabel>Reasign to</DropdownMenuLabel>
                                                        <DropdownMenuSeparator />
                                                        <div class="p-2">
                                                            <Input v-model="reassignSearch[member.id]"
                                                                @input="debounceFetchAssigneeSuggestions(member.id)"
                                                                placeholder="Search name or division"
                                                                class="w-full h-9 text-sm" />
                                                        </div>
                                                        <div class="max-h-64 overflow-auto">
                                                            <div v-if="assigneeLoading[member.id]"
                                                                class="px-3 py-2 text-sm text-muted-foreground">
                                                                Loading...</div>
                                                            <DropdownMenuItem
                                                                v-for="a in currentAssigneeOptions(member)" :key="a.id"
                                                                @click.prevent="reassignMember(member.id, a.id)"
                                                                class="flex justify-between items-center gap-2 py-2 px-3">
